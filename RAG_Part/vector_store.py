...
# 注意：必须让 rag_dir 排在 project_root 前面，否则 from config import Config
# 可能解析到 My_agent/config.py（缺少 RAG_Part 的检索参数）而报错
...
def _sparse_row_indices_values(row):
    """提取稀疏向量一行的 (非零列索引, 非零值)。
    新版 scipy/milvus-model 整体返回 csr_array，按行取出可能是 coo_array，
    二者取索引的方式不同，这里统一兼容处理。
    """
    if hasattr(row, "indices"):
        return row.indices, row.data
    coo = row.tocoo()
    return coo.col, coo.data
...
class VectorStore:
    ...
    def __init__(self,
        ...
        # 日志提醒使用的是什么设备
        print(f"使用设置：{self.device}")
        # 初始化 BGE-Reranker 模型，用于重排序检索结果
        reranker_path = conf.bge_reranker_model_path
        # print(f'reranker_path--》{reranker_path}')
        self.reranker = CrossEncoder(reranker_path, device=self.device)
        # 初始化 BGE-M3 嵌入函数
        # use_fp16 固定为 False：集合 dense_vector 字段为 FLOAT_VECTOR(float32)，
        # 若在 cuda 上用 fp16 会产生 float16 向量，检索时与字段类型不匹配而报错
        m3_path = conf.bge_m3_model_path
        self.embedding_function = BGEM3EmbeddingFunction(model_name_or_path=m3_path, use_fp16=False, device=self.device)
        # 获取稠密向量的维度# 1024
    ...
    def _create_or_load_collection(self):
        ...
        # 如果集合已存在
            ...
            # 记录加载集合的日志
            print(f"已加载集合 {self.collection_name}")
        # 将集合加载到内存，确保可立即查询
    ...
    def add_documents(self, documents):
        ...
        for i, doc in enumerate(documents):
            ...
            # 新版 milvus-model/scipy 用下标 [i] 取行（csr_matrix 才支持 getrow）
            row = embeddings["sparse"][i]
            # 获取稀疏向量的非零值索引与值（兼容 csr/coo 格式）
            indics, values = _sparse_row_indices_values(row)
            # 将索引和值进行配对，存储到字典中
            for idx, value in zip(indics, values):
                sparse_vector[int(idx)] = float(value)
            # Milvus 的 SPARSE_FLOAT_VECTOR 不允许空向量，跳过无有效内容的退化子块
            if not sparse_vector:
                print(f"⚠️ 跳过空稀疏向量子块（无有效内容）: {doc.page_content[:50]!r}")
                continue
            # 创建数据字典，包含所有字段
            entry = {
                "id": text_hash,
            ...
            # 动态字段：保留完整元数据（集合已启用 enable_dynamic_field），
            # 便于检索后还原图片路径、表格 HTML 等信息
            for k in ("type", "category", "heading_path", "source_file",
    ...
    def hybrid_search_with_rerank(self, query, k=conf.RETRIEVAL_K, source_filter=None):
        ...
        # 初始化查询的稀疏向量字典
        sparse_query_vector = {}
        # 获取查询稀疏向量的第 0 行数据（csr_array 用下标取行）
        row = query_embeddings["sparse"][0]
        # 获取稀疏向量的非零值索引与值（兼容 csr/coo 格式）
        indices, values = _sparse_row_indices_values(row)
        # 将索引和值配对，填充稀疏向量字典
        for idx, value in zip(indices, values):
            sparse_query_vector[int(idx)] = float(value)
